...
# Persist defaults so they are visible/tunable in config.
python3 - "$CONFIG" <<'PY'
...
sed -i -E 's#ExecStart=/usr/bin/python3 /opt/xhttp-dual/controller(-v2|-v3)?\.py daemon#ExecStart=/usr/bin/python3 /opt/xhttp-dual/controller-v3.py daemon#' "$SERVICE"
...
echo "Slow-path limit : 1500 ms end-to-end through each XHTTP tunnel"
echo "Slow path       : 2 consecutive slow probes => unhealthy"
echo "Hard failure    : 3 consecutive failed probes => unhealthy"
echo "Recovery        : 5 consecutive good probes => healthy"
