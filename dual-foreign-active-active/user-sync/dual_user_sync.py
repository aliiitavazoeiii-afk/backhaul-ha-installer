...
class Panel:
    def __init__(self, name, cfg):
        ...
        if self.base.startswith("https://"):
            ctx = (
                ...
                if self.verify_tls else ssl._create_unverified_context()
            )
    ...
    def request(self, method, path, payload=None, form=False):
        ...
        try:
            ...
            raise RuntimeError(
                f"{self.name}: HTTP {e.code} {path}: {body[:400]}"
            ) from e
        ...
        try:
            ...
            raise RuntimeError(
                f"{self.name}: non-JSON response from {path}: {raw[:400]}"
            ) from e
    ...
    def login(self):
        ...
        if not res.get("success"):
            raise RuntimeError(
                f"{self.name}: login failed: {res.get('msg', 'unknown error')}"
            )
    ...
    def list_inbounds(self):
        ...
        if not res.get("success") or not isinstance(res.get("obj"), list):
            raise RuntimeError(
                f"{self.name}: cannot list inbounds: "
                f"{res.get('msg', 'invalid response')}"
            )
        return res["obj"]
    ...
    def add_client(self, inbound_id, client):
        ...
        body = {
            "id": int(inbound_id),
            "settings": json.dumps({"clients": [payload]}, separators=(",", ":"))
        }
        res = self.request("POST", "panel/api/inbounds/addClient", body)
        if not res.get("success"):
            raise RuntimeError(
                f"{self.name}: addClient failed for {client.get('email', '?')}: "
                f"{res.get('msg', 'unknown error')}"
            )
    ...
    def update_client(self, inbound_id, existing_auth_id, client):
        ...
        res = self.request(
            "POST", f"panel/api/inbounds/updateClient/{cid}", body
        )
        if not res.get("success"):
            raise RuntimeError(
                f"{self.name}: updateClient failed for {client.get('email', '?')}: "
                f"{res.get('msg', 'unknown error')}"
            )
    ...
    def delete_client(self, inbound_id, existing_auth_id, email="?"):
        cid = urllib.parse.quote(str(existing_auth_id), safe="")
        res = self.request(
            "POST", f"panel/api/inbounds/{int(inbound_id)}/delClient/{cid}", {}
        )
        if not res.get("success"):
            raise RuntimeError(
                f"{self.name}: delClient failed for {email}: "
                f"{res.get('msg', 'unknown error')}"
            )
...
def parse_clients(inbound):
    ...
    try:
        ...
        raise RuntimeError(
            f"Inbound {inbound.get('id')} has invalid settings JSON: {e}"
        ) from e
    clients = settings.get("clients", [])
    if not isinstance(clients, list):
        raise RuntimeError(
            f"Inbound {inbound.get('id')} settings.clients is not a list"
        )
    return clients
...
def select_inbound(inbounds, cfg, peer_protocol=None):
    ...
    if configured not in (None, "", 0, "0"):
        for inbound in inbounds:
            if int(inbound.get("id", -1)) == int(configured):
                if peer_protocol and (
                    str(inbound.get("protocol", "")).lower()
                    != str(peer_protocol).lower()
                ):
                    raise RuntimeError(
                        f"Configured inbound_id={configured} protocol mismatch"
                    )
                return inbound
    ...
    if not candidates:
        ...
        raise RuntimeError(
            f"No inbound matched listen={target_listen!r} "
            f"port={target_port}{suffix}"
        )
    ids = ",".join(str(x.get("id")) for x in candidates)
    raise RuntimeError(
        f"Multiple inbounds matched ({ids}); set inbound_id explicitly"
    )
...
def discover(cfg):
    ...
    ib = select_inbound(
        pb.list_inbounds(), cfg["secondary"], str(ia.get("protocol", ""))
    )
    return pa, pb, ia, ib
...
def validate_delete_plan(cfg, primary_count, secondary_count, extras):
    ...
    if primary_count == 0 and secondary_count > 0 and not guard["allow_empty_primary"]:
        raise RuntimeError(
            "delete guard blocked sync: primary returned zero clients; "
            "intentional full wipe requires delete_guard.allow_empty_primary=true"
        )
    count = len(extras)
    fraction = count / max(secondary_count, 1)
    if count > int(guard["max_delete_count"]):
        raise RuntimeError(
            f"delete guard blocked sync: {count} deletes > "
            f"max_delete_count={guard['max_delete_count']}"
        )
    if fraction > float(guard["max_delete_fraction"]):
        raise RuntimeError(
            f"delete guard blocked sync: {fraction:.1%} delete fraction > "
            f"max_delete_fraction={float(guard['max_delete_fraction']):.1%}"
        )
...
def verify(source, secondary, exact):
    ...
    mismatch = [
        k for k in source
        if k in secondary and canonical(source[k]) != canonical(secondary[k])
    ]
    extras = [k for k in secondary if k not in source]
    if mismatch:
        for key in mismatch[:5]:
            warn(
                f"Mismatch {key}: fields="
                f"{','.join(differing_fields(source[key], secondary[key]))}"
            )
    if missing or mismatch:
        raise RuntimeError(
            f"Verification failed: missing={len(missing)} mismatch={len(mismatch)}"
        )
    if exact and extras:
        raise RuntimeError(
            f"Verification failed: {len(extras)} extra client(s) remain on B"
        )
    ...
    if exact:
        ...
        ok(
            f"Primary-set hash verified on B: {source_hash[:16]}... "
            f"(extras_on_B={len(extras)})"
        )
...
def run_sync(cfg, dry_run=False):
    ...
    if proto != str(ib.get("protocol", "")).lower():
        raise RuntimeError(
            f"Protocol mismatch: A={proto} B={ib.get('protocol')}"
        )
    ac = index_clients(proto, parse_clients(ia))
    bc = index_clients(proto, parse_clients(ib))

    info(
        f"Primary A inbound={ia.get('id')} {ia.get('listen')}:{ia.get('port')} "
        f"protocol={proto} clients={len(ac)}"
    )
    info(
        f"Secondary B inbound={ib.get('id')} {ib.get('listen')}:{ib.get('port')} "
        f"protocol={proto} clients={len(bc)}"
    )

    adds = [k for k in ac if k not in bc]
    updates = [
        k for k in ac
        if k in bc and canonical(ac[k]) != canonical(bc[k])
    ]
    extras = [k for k in bc if k not in ac]
    delete_enabled = bool(cfg.get("delete_mirroring", False))
    info(
        f"Plan: add={len(adds)} update={len(updates)} "
        f"extra_on_B={len(extras)} "
        f"delete_mirroring={'ON' if delete_enabled else 'OFF'}"
    )
    ...
    if dry_run:
        ...
        for key in updates:
            print(
                f"UPDATE {key} "
                f"fields={','.join(differing_fields(ac[key], bc[key]))}"
            )
        for key in extras:
            print(f"EXTRA  {key}")
    ...
    if delete_enabled:
        for key in extras:
            ...
            if not old_id:
                raise RuntimeError(
                    f"Cannot identify existing auth id for deletion: {key}"
                )
...
def main():
    ap = argparse.ArgumentParser(
        description="Mirror 3x-ui inbound clients from Foreign A to Foreign B"
    )
