...
def normalize_client(client):
    ...
    if "enable" in c:
        v = c["enable"]
        if isinstance(v, str):
            c["enable"] = v.strip().lower() in ("1", "true", "yes", "on")
        else:
            c["enable"] = bool(v)
    return c
...
def canonical(client):
    return json.dumps(normalize_client(client), sort_keys=True, separators=(",", ":"), ensure_ascii=False)


def differing_fields(a, b):
    aa, bb = normalize_client(a), normalize_client(b)
    keys = sorted(set(aa) | set(bb))
    return [k for k in keys if aa.get(k) != bb.get(k)]
...
class Panel:
    ...
    def request(self, method, path, payload=None, form=False):
        headers = {"User-Agent": "dual-user-sync/0.2"}
        data = None
...
def select_inbound(inbounds, cfg, peer_protocol=None):
    ...
    for i in inbounds:
        if int(i.get("port", -1)) != target_port:
            continue
        if target_listen and str(i.get("listen", "") or "") != target_listen:
            continue
    ...
    raise RuntimeError("Multiple matching inbounds; set inbound_id explicitly")
...
def stable_key(protocol, client):
    ...
    return "email:" + email if email else "auth:" + str(auth_id(protocol, client))
...
def run_sync(cfg, dry_run=False):
    ...
    if dry_run:
        for k in adds:
            print(f"ADD    {k}")
        for k in updates:
            print(f"UPDATE {k} fields={','.join(differing_fields(ac[k], bc[k]))}")
        for k in extras:
            print(f"EXTRA  {k}")
        return 0

    for k in adds:
        pb.add_client(ib["id"], ac[k]); ok(f"Added on B: {k}")
    for k in updates:
        old_id = auth_id(proto, bc[k])
        if not old_id:
            raise RuntimeError(f"Cannot identify existing auth id for {k}")
        pb.update_client(ib["id"], old_id, ac[k]); ok(f"Updated on B: {k}")
    if delete_enabled:
        for k in extras:
            old_id = auth_id(proto, bc[k])
            if not old_id:
                raise RuntimeError(f"Cannot identify existing auth id for deletion: {k}")
            pb.delete_client(ib["id"], old_id, bc[k].get("email", "?")); ok(f"Deleted from B: {k}")
    elif extras:
    ...
    if mismatch:
        for k in mismatch[:5]:
            warn(f"Mismatch {k}: fields={','.join(differing_fields(ac[k], bc2[k]))}")
