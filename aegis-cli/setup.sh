...
cleanup_old_dashboard(){
    if systemctl list-unit-files 2>/dev/null | grep -q '^aegis-dashboard.service'; then
        log 'Removing the old Aegis web dashboard from the control path'
...
check_hostkey_conflict(){
    local ip="$1" out rc
    set +e
    out="$(ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=yes "root@$ip" true 2>&1)"
    rc=$?
    set -e
    if grep -q 'REMOTE HOST IDENTIFICATION HAS CHANGED' <<<"$out"; then
        printf '%s\n' "$out" >&2
        die "SSH host key for $ip changed. Verify the server in your provider console, then run: ssh-keygen -f /root/.ssh/known_hosts -R $ip"
    fi
    return "$rc"
}

bootstrap_ssh(){
    local ip="$1" label="$2" pw
    ssh_ready "$ip" && { ok "$label SSH key already works"; return; }
    check_hostkey_conflict "$ip" || true
    printf '\nRoot password for %s (%s) [used once, not saved]: ' "$label" "$ip"
    read -rs pw
    printf '\n'
    [[ -n "$pw" ]] || die "Empty password for $label"
    export SSHPASS="$pw"
    sshpass -e ssh-copy-id -i "${SSH_KEY}.pub" -o StrictHostKeyChecking=accept-new "root@$ip" >/dev/null
    unset SSHPASS pw
    ssh_ready "$ip" || die "SSH key bootstrap failed for $label $ip"
...
if [[ -n "$existing_ver" && "$existing_ver" != "$VERSION" ]]; then
    die "existing 3x-ui version is $existing_ver; refusing an automatic downgrade to $VERSION"
fi
...
if [[ "$MODE" == 'backup' ]]; then
    [[ -s "$TEMPLATE" ]] || die 'primary DB template missing on backup'
    sqlite3 "$TEMPLATE" 'PRAGMA quick_check;' | grep -qx ok || die 'primary DB template is not healthy'
    if [[ -s "$DB" ]]; then
    ...
    sleep 4
    systemctl is-active --quiet x-ui || die 'x-ui failed after restoring primary DB'
...
# PRIMARY path. Preserve a healthy existing v2.9.4 installation.
...
# Fresh/empty primary: initialize DB, secure the local panel, and create one
# bootstrap VLESS/TCP inbound on 127.0.0.1:443 so the tunnel is immediately testable.
if [[ -s "$DB" ]]; then
    sqlite3 "$DB" ".backup '${BACKUP_DIR}/x-ui.db.$(date +%Y%m%d-%H%M%S)'" || cp -a "$DB" "${BACKUP_DIR}/x-ui.db.raw.$(date +%Y%m%d-%H%M%S)"
fi
...
systemctl restart x-ui
sleep 4
systemctl is-active --quiet x-ui || die 'fresh x-ui service failed to start'
...
LOGIN_RESP="$(curl -fsS -c "$COOKIE" -X POST -d "username=${USER}&password=${PASS}" "$LOGIN" || true)"
grep -q '"success":true' <<<"$LOGIN_RESP" || die 'could not authenticate to fresh local 3x-ui panel'
...
print(json.dumps({
 ...
 'expiryTime':0,'trafficReset':'never','lastTrafficResetTime':0,'listen':'127.0.0.1',
 'port':443,'protocol':'vless','settings':sys.argv[1],'streamSettings':sys.argv[2],
 'sniffing':sys.argv[3]
}))
PY
)"
RESP="$(curl -fsS -b "$COOKIE" -H 'Content-Type: application/json' -X POST --data "$PAYLOAD" "$API" || true)"
grep -q '"success":true' <<<"$RESP" || { printf '%s\n' "$RESP" >&2; die '3x-ui rejected the bootstrap inbound'; }
systemctl restart x-ui
sleep 5
...
bootstrap_note=VLESS TCP no-TLS bootstrap user for tunnel testing; replace/add production clients in 3x-ui as needed.
EOF
chmod 0600 "$CREDS_DIR/panel.txt"
log 'fresh primary 3x-ui v2.9.4 + bootstrap Xray :443 is UP'
REMOTE
}

run_remote_xui(){
    local ip="$1" mode="$2" template_remote="${3:-}"
    remote_xui_script | ssh "${SSH_OPTS[@]}" "root@$ip" "bash -s -- '$mode' '$IRAN_IP' '$template_remote'"
}
...
EOF
    chmod 0600 "$STATE_DIR/config.env"
    {
        ...
        echo 'Future clients must exist on both Foreign 3x-ui databases if they need failover.'
    } > "$RESULT_FILE"
...
main(){
    ...
    read -rp 'Aegis domain (must A-record to this IRAN IP): ' DOMAIN
    ...
    printf 'Domain:  %s\n' "$DOMAIN"
    printf '\nSaved summary: %s\n' "$RESULT_FILE"
    printf '\nNo web dashboard is required. Re-run this wizard when replacing servers.\n'
}
